...
const MlpLab: React.FC<LabKitProps> = ({ descriptor, tutor, apiPanel }) => {
  ...
  const step = () => {
    ...
    const introSentence = `The challenge here: separate the ${kind} pattern, where the two classes are tangled in a shape that no single straight line could ever split. This multilayer perceptron with ${hlayers} hidden ${hlayers === 1 ? 'layer' : 'layers'} of ${hidden} ${act} neurons takes a weighted sum at each layer, adds a bias, and bends it through ${act}, so the hidden layers warp the input space until one final straight cut can separate the classes. Backprop sends the error backward by the chain rule and the weights step downhill using ${optWords}${l2 > 0 ? ', while the L2 penalty gently shrinks the weights toward zero for a smoother boundary' : ''}. Watch the coloured decision region bend around the points and the network edges light up as the weights, teal for positive and red for negative, take shape. This same pattern of stacked layers trained by backprop is the engine behind modern image recognition, speech systems and the large language models in everyday use.`;
