...
const PerceptronLab: React.FC<LabKitProps> = ({ descriptor, tutor, apiPanel }) => {
  ...
  const step = () => {
    ...
    const introSentence = rule === 'margin'
      ? `The challenge here: separate the two classes of dots with a single straight line, but leave a clear safety gap on both sides instead of just barely splitting them. This margin perceptron cycles through the points and, whenever one sits inside a band of width gamma around the line, nudges the weights by the learning rate times the label times the point, pushing it a clear distance off the boundary. Watch the white line and its gold margin lines settle as the dots clear the band. Demanding a wide margin like this is the core idea behind support vector machines, still used for text and image classification.`
      : rule === 'pocket'
