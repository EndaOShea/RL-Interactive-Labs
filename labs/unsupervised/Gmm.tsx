...
const GmmLab: React.FC<LabKitProps> = ({ descriptor, tutor, apiPanel }) => {
  ...
  const step = () => {
    ...
    narration.narratePhase(
      `run:${covType}:${K2}`,
      `The challenge here: explain these overlapping blobs as a mixture of ${K2} Gaussian sources and decide, softly, which source each point most likely came from, rather than forcing the hard circular splits that k-means would. A Gaussian mixture fitted by expectation maximisation solves it by alternating two moves: the E step gives every point a soft responsibility, the probability that it belongs to each component, and the M step refits each blob to its responsibility-weighted points. With ${covType} covariance the blobs are ${covType === 'spherical' ? 'plain circles, essentially soft k-means' : covType === 'diag' ? 'axis-aligned ellipses that stretch but do not rotate' : 'full ellipses that can stretch and rotate to follow correlated data'}. Watch the ellipses and the log-likelihood, which EM is guaranteed never to decrease. Mixtures like this drive speaker recognition and audio modelling, image and colour segmentation, and density estimation for anomaly detection.`,
    );
    if (converged) {
      narration.narratePhase(
        `done:${covType}:${K2}`,
