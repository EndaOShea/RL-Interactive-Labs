...
const FeatureMapsLab: React.FC<LabKitProps> = ({ descriptor, tutor, apiPanel }) => {
  ...
  const introNarration =
    `The challenge here: recognise which letter the glyph ${cls} is, working only from raw pixels. A tiny convolutional pipeline tackles it in stages — convolution, then ReLU, then pooling, then a classifier. Three fixed filters — a vertical edge, a horizontal edge, and a blob — slide over the image, and each produces a feature map showing where its pattern occurs. That is the formula I times K, a weighted sum of the patch under each filter. Here the ${peakFilter.name} filter fires hardest. Then ReLU, the maximum of zero and the value, keeps only the positive responses, so each map shows where its feature is actually present. Watch the three feature maps light up. This conv-ReLU-pool stack is the backbone of the convolutional networks behind handwriting and document recognition, object detection and medical image analysis.`;
  const midNarration =
    poolMode === 'max'
