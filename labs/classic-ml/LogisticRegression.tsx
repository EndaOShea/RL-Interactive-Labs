...
const LogisticRegressionLab: React.FC<LabKitProps> = ({ descriptor, tutor, apiPanel }) => {
  ...
  const step = () => {
    ...
    const intro = l2 > 0
      ? `The challenge here: find the straight line that best separates the two classes of points and reports a calibrated probability for each side. Logistic regression passes a weighted sum of the features through the sigmoid to get that probability, then minimises cross-entropy plus a penalty lambda times the squared weights. On separable data that penalty matters: without it the weights run off to infinity, so the penalty caps the weight norm and keeps the probabilities sane. Watch the solid white line, where the probability is one half, and the dashed quarter and three-quarter band narrow as the weight norm grows. This is a staple of credit scoring, medical diagnosis, spam filtering, and click-through prediction.`
      : `The challenge here: find the straight line that best separates the two classes of points and reports a probability for each side. Logistic regression passes a weighted sum of the features through the sigmoid — one over one plus e to the minus z — turning the score into a probability, and trains by minimising cross-entropy. Each step moves the weights along the clean gradient, the average of the prediction minus the label times the input. Watch the solid white line, where the probability is one half, tilt to split the two classes, with the dashed quarter and three-quarter contours forming the confidence band. This is a staple of credit scoring, medical diagnosis, spam filtering, and click-through prediction.`;
