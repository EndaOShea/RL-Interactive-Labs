...
for episode in range(200):
  ...
  const step = useCallback(() => {
    ...
    const intro: Record<string, string> = {
        q: 'The challenge: the agent starts knowing nothing about this grid and must learn, from trial and reward alone, a route from the start to the goal that dodges the penalty tiles. Q-learning is value-based and off-policy: each step it nudges the quality of an action toward the reward plus the discounted best value of the next square. Watch the heat map fill in as good paths back up their value toward the start. This is the backbone of reinforcement learning in robot control, game-playing agents and recommendation systems.',
        sarsa: 'Same challenge — learn a safe path from start to goal from experience alone — but SARSA is on-policy: it backs up the value of the action it actually takes next, exploration mistakes and all, so its value map stays a little more cautious than Q-learning around the obstacles. Watch the heat spread along the path it really walks. On-policy methods like this are used wherever an agent must learn safely while it acts, as in industrial control and robotics.',
        esarsa: 'Same task: discover a grid route from reward alone. Expected SARSA backs up the average value over the whole epsilon-greedy distribution instead of one sampled next action — the same target as SARSA, but far smoother, so the heat map settles with less jitter. Watch the values climb steadily toward the goal. Its stability makes it a common building block inside larger deep-RL systems.',
        doubleq: 'Same challenge: find the best path by trial and reward. Double Q-learning keeps two value tables — one picks the best next action, the other scores it — so noisy luck cannot confirm itself, removing the optimistic bias of plain Q-learning. Watch the heat grow calmer and less inflated. This exact bias fix powers modern deep-RL agents like Double DQN used in game-playing and control.',
        reinforce: 'The agent must learn a route from start to goal from reward alone. REINFORCE is a Monte-Carlo policy gradient: it learns instructions, not values, and only updates at the end of an episode, using the total discounted return to push up the probability of every action it took. Watch the policy arrows emerge before any heat does. Policy-gradient methods like this train robotics, continuous control and the reinforcement-learning fine-tuning behind modern language models.',
        ac: 'Same goal — learn a grid path purely from experience. Actor-Critic blends both ideas: the critic learns how good each state is, and its surprise, the temporal-difference error, tells the actor whether to make the chosen action more or less likely. Watch the value heat and the policy arrows grow together. Actor-Critic underlies workhorse algorithms like A3C and PPO used across robotics and game AI.',
        dyna: 'The agent must find a path from start to goal, but with as few real moves as possible. Dyna-Q is model-based: it learns a model of the world and then dreams, replaying remembered transitions to back up value without real moves, so it learns from far less experience. Watch the purple planning flashes spread value across the grid. Model-based planning like this drives sample-efficient robotics and systems such as AlphaZero.',
    };
...
for episode in range(100):
    ...
    const step = useCallback(() => {
        ...
        if (policyType === 'deterministic') {
            intro = slipOn
                ? 'The challenge: reach the goal when your own actions can misfire. This is a deterministic policy — it always takes the single highest-value action — but the world is stochastic now, so the agent sometimes slips sideways even when it chooses well. Watch how a rigid policy can be punished for outcomes it did not actually choose, and why a high learning rate then becomes risky. This is the core question behind controlling robots and vehicles whose actuators and sensors are never perfectly reliable.'
                : 'The challenge: reach the goal in a world where every move plays out exactly as intended. This is a deterministic policy in a clean, predictable environment — it simply takes the highest-value action every time, and one observation is the truth. Watch it commit to a single sharp route toward the goal. Deterministic policies like this suit well-modelled settings such as board games and tightly controlled automation.';
        } else {
            intro = slipOn
                ? 'The challenge: stay robust when actions can slip. This is a stochastic, softmax policy in a noisy world: it samples actions by a Boltzmann distribution over their values, warmer temperatures explore more and cooler ones sharpen toward greedy. Because it keeps its options open, it copes with slips more gracefully than a rigid policy. Watch the temperature cool over episodes. Randomised policies like this matter in robotics, finance and any control problem with real-world noise.'
                : 'The challenge: balance trying new routes against taking the best known one. This is a stochastic, softmax policy: instead of always taking the best action, it samples in proportion to value, with temperature controlling how random the choice is. Watch it explore several routes before settling toward the goal. Stochastic policies are the foundation of policy-gradient methods used in modern robotics and language-model training.';
        }
        if (done) {
...
for episode in range(200):
    ...
    const step = useCallback(() => {
        ...
        let intro: string;
        if (mode === 'tabular') {
            intro = 'The challenge: how do you store the value of every state, and what happens when there are too many to store? This is tabular learning: one value per square, looked up exactly. Each update touches only the cell you are in, so learning is precise but it never generalizes — knowing one square tells you nothing about its neighbours. Watch the heat appear one cell at a time. Exact tables work for small problems but break down for the huge state spaces of real games and robotics.';
        } else if (featureType === 'tile') {
            intro = 'The challenge: cover a state space too large for one value per cell. This is deep, approximate learning with tile coding: instead of one value per square, the agent learns weights on features that cover blocks of cells, so each update spreads across a whole tile at once. Learning generalizes and speeds up, but sharp tile edges can blur the true values. Watch a single lesson light up a block of squares. Tile coding was a classic way to scale RL before neural networks took over.';
        } else {
            intro = 'The challenge: generalize from a few visits to a vast state space. This is deep, approximate learning with radial basis features, a stand-in for a neural network: each update bleeds smoothly into nearby states by a Gaussian, so the agent generalizes from a few visits to many squares. That is faster but can introduce errors where the smoothing is wrong. Watch one lesson glow outward to its neighbours. Function approximation like this is exactly what lets deep RL handle images, sensors and continuous control.';
        }
        if (done) {
...
for t in range(1, 501):
    ...
    const step = useCallback(() => {
        ...
        const intro: Record<string, string> = {
            ...
            epsilon: 'Same challenge: discover the highest-paying arm while spending as little as possible finding out. Epsilon-greedy mostly pulls its current best arm but, with probability epsilon, tries a random arm instead. That small constant exploration is usually enough to discover the true best arm. Watch the estimates on each bar climb toward their hidden true rates. Epsilon-greedy is the simplest workhorse behind recommendation and online-advertising systems.',
            optimistic: 'Same challenge: identify the best slot machine while minimising wasted pulls. Optimistic initial values start every arm with an inflated estimate, so disappointment from each pull drives the agent to try every arm at least once — exploration happens automatically without randomness. Watch the inflated bars deflate toward their real values. Optimism in the face of uncertainty is a guiding principle across modern exploration algorithms.',
            ucb: 'Same challenge: maximise total payout from machines whose odds you must learn. Upper Confidence Bound adds an exploration bonus to each estimate that grows for arms tried less often, then picks the arm with the best optimistic upper bound, balancing what looks good against what is still uncertain. Watch rarely-pulled arms get revisited as their uncertainty bonus rises. UCB has strong guarantees and shows up in ad placement, recommendation and Monte-Carlo tree search.',
            thompson: 'Same challenge: find the best-paying arm while keeping regret low. Thompson sampling keeps a probability distribution of belief for each arm and pulls the arm that wins a random draw from those beliefs, exploring arms it is unsure about in proportion to the chance they are best. Watch the belief over the strongest arm sharpen as evidence accumulates. This Bayesian approach is widely used in industrial A/B testing and online recommendation.',
            boltzmann: 'Same challenge: concentrate pulls on the best machine without ignoring the rest too soon. Boltzmann, or softmax, exploration pulls each arm with probability proportional to the exponential of its estimated value over a temperature — higher value means more pulls, but every arm keeps some chance. Watch the pulls concentrate on the better bars over time. The same softmax-over-value idea reappears in reinforcement-learning policies and decision-making systems.',
        };
...
for episode in range(100):
    ...
    const step = useCallback(() => {
        ...
        const intro: Record<string, string> = {
            ...
            coop: 'The challenge: get two agents to reach their goals together when each one\'s best move depends on the other. This is cooperative multi-agent learning — both agents key their values on the joint state, the pair of positions, and share their rewards, so the hard part is credit assignment: deciding which agent earned the team success. Watch them learn to coordinate. This is the core problem in warehouse robot fleets, sensor networks and team game AI.',
            comp: 'The challenge: learn to win against an opponent who is learning to beat you back. This is a competitive, zero-sum chase — the predator is rewarded for closing in and the prey for escaping, and each learns over the joint state of both positions. Because the opponent is also adapting, the environment is non-stationary, a moving target. Watch the strategies co-evolve. Self-play between competing agents is exactly how systems like AlphaGo and poker and StarCraft AIs were trained.',
            congestion: 'The challenge: share one goal without getting in each other\'s way. This is a congestion or social-dilemma scenario — both agents want the same shared cell, but colliding there hurts them, so greedy self-interest can hurt the collective. Each reasons over the joint state. Watch whether they learn to take turns rather than clash. These dynamics drive traffic routing, autonomous-vehicle coordination and network-resource sharing.',
        };
